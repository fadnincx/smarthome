...
bool MQTTClient::publish(char *topic, char *data, byte qos, bool retain, bool duplicate) {
  ...
  if ((topic != NULL) && (strlen(topic)>0) && (qos<3) && (isConnected)) {
    ...
    return result;
     ...
     if (!(topic != NULL)){
       Serial.println("mqtt.h > Publish > Topic null!");
     }else if (!(strlen(topic)>0)){
       Serial.println("mqtt.h > Publish > Topic strlen <= 0!");
     }else if (!(qos<3)){
       Serial.println("mqtt.h > Publish > QOS >= 3");
     }else if (!(isConnected)){
       Serial.println("mqtt.h > Publish > not Connected");
     }else{
       Serial.println("mqtt.h > Publish > Unknown error");
     }
